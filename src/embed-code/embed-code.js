...
function getIFrameEmbedCodeWithInlineResize(baseUrl, embedUrl) {
  const id = getIFrameId();
  const url = getEmbedUrl(baseUrl, embedUrl);
  return getEmbedCodeIFrame(id, url) + getEmbedCodeResizeScript(id);
}


function getIFrameEmbedCodeWithRemoteResize(baseUrl, embedUrl) {
  const id = getIFrameId();
  const url = getEmbedUrl(baseUrl, embedUrl);
...
function getScriptTagEmbedCode(baseUrl, embedUrl) {
...
function getEmbedUrl(baseUrl, embedUrl) {
  if (!embedUrl) {
    return baseUrl;
  }
  return embedUrl;
...
function getEmbedCodeIFrame(id, src) {
  return sprintf('<iframe id="%s" src="%s" width="100%%" scrolling="no" frameborder="0"></iframe>',
    id, src);
};


function getEmbedCodeRemoteResizeScript(baseUrl, id) {
...
function getEmbedCodeResizeScript(id) {
